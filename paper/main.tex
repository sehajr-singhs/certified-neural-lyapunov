...
relu/exp ELU identity, for the delta-complete SMT solver dReal, and check the
negation, whether any state in the region has a nonnegative Lie derivative, on the
4-D two-bus gate and on the gen-5 slice. The encoding is complete and included, and
the measurement is pending, so we state the comparison as provided but not yet run
and we report no wall-clock we did not take.
