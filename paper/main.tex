...
relu/exp ELU identity, for dReal, and check the negation, whether any state in the
region has a nonnegative Lie derivative. dReal ships no Windows wheel and its source
build needs Bazel and IBEX, so it does not run on the Windows host this work was
developed on, the same environment wall the IBM CROWN repository hit, and the
encoding is provided to run on Linux where the expected outcome from the SMT
scalability literature is that the 4-D two-bus gate closes and the NE39 slice
exceeds its budget while CROWN certifies it in seconds. We report the encoding here
and do not report a dReal wall-clock we did not measure.
